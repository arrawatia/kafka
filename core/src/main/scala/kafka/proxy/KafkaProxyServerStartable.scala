...
object KafkaProxyServerStartable {
  def fromProps(serverProps: Properties) = {
    KafkaMetricsReporter.startReporters(new VerifiableProperties(serverProps))
    new KafkaProxyServerStartable(KafkaConfig.fromProps(serverProps))
  }
}



class KafkaProxyServerStartable(val serverConfig: KafkaConfig) extends Logging {
  private val server = new KafkaProxyServer(serverConfig)
